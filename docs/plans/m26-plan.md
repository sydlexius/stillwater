...
- [x] `existingImageFileNames` works correctly on case-sensitive filesystems
- [x] MusicBrainz mirror research findings documented with actionable recommendations
- [ ] Artist grid shows blurred placeholders when images are unavailable
- [x] Genius provider fetches artist biographies via Genius API
- [x] Spotify provider fetches artist images, genres, and popularity via Spotify Web API
...
- [x] Case-insensitive file existence check in `internal/rule/fixers.go`
- [x] Tests for mixed-case filenames on case-sensitive filesystems
- [x] PR opened (#329)
- [x] CI passing
- [x] PR merged

### Issue #315 -- Research: MusicBrainz mirror support
- [x] Survey available community mirrors and self-hosting options
- [x] Document rate limit implications for mirrors
- [x] Assess WS/2 API compatibility across mirrors
- [x] Document configuration UX (base URL field already supported)
- [x] Findings posted to issue
- [x] Issue closed (PR #332)
...
- [x] New package `internal/provider/genius/`
- [x] Implement `Provider` interface (SearchArtist, GetArtist; GetImages returns empty)
- [x] Bearer token authentication
- [x] Map Genius artist description to `ArtistMetadata.Biography`
- [x] Settings UI: Genius card with API token field
- [x] Register in provider registry
- [x] Tests
- [x] PR opened (#334)
- [x] CI passing
- [x] PR merged
- [x] Docs updated (Architecture wiki: new provider)

### Issue #304 -- Add Spotify as a metadata and image provider
- [x] New package `internal/provider/spotify/`
- [x] Implement `Provider` interface (SearchArtist, GetArtist, GetImages)
- [x] OAuth 2.0 Client Credentials flow (client ID + secret)
- [x] Map Spotify genres, popularity, images to Stillwater types
- [x] Settings UI: Spotify card with client ID and secret fields
- [x] Register in provider registry
- [x] Tests
- [x] PR opened (#333)
- [x] CI passing
...
- 2026-03-02: 4 of 5 issues completed. Only #311 (LQIP placeholders) remains open on branch `feat/311-lqip-placeholders`. Plan file will be deleted once #311 merges.
