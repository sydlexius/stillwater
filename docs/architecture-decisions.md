...
**Why `mem_reservation` is set at 512m:** It is a soft memory limit enforced by the kernel under host memory contention. It does not reserve capacity or make placement decisions in plain Compose. When memory is plentiful the container may exceed it, up to `mem_limit`; when the host is under memory pressure, the kernel tries to push the container toward this figure. 512m is a conservative chosen value well below the transient peak, not a measured steady-state working set.
...
**Why service-level compose keys, not `deploy.resources`:** This file uses the service-level Compose Spec keys (`mem_limit`, `mem_reservation`, `cpus`) because they work in plain `docker compose up`. Modern Docker Compose also supports `deploy.resources` outside Swarm, but the service-level keys are placed where the documented install path applies them. If both forms are declared, they must be consistent with each other.
