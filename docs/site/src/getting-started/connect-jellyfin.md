...
- **Image write.** Stillwater pushes artwork (primary, backdrop, banner, logo) to Jellyfin via its API. Replacing the primary fanart works a little differently on Jellyfin than on Emby -- see [Replacing the primary fanart on a connected platform](../core-concepts/images.md#replacing-the-primary-fanart-on-a-connected-platform).
...
- **Automatic link repair after a directory rename.** When you rename an artist's directory, Stillwater asks Jellyfin to rescan and then re-reads the artist back at its new path so its stored link keeps pointing at the right item. On a large library that rescan can take minutes, which is longer than the rename itself waits, so the link is left as-is and the rename is reported as needing attention. A scheduled background job (default cadence: every 15 minutes) picks those up afterward. Each run re-checks every linked artist's Jellyfin mapping, not just the ones still waiting to be confirmed -- a resolved link costs one comparison against the connection's cached listing, so the per-run cost scales with your library size, not with how many links are currently pending. It only ever moves a link to an item Jellyfin itself reports at the artist's current path, and it never removes a link, so an artist that still cannot be matched is left alone and retried on the next pass. Configure the cadence via the `relink_reconcile.interval_minutes` setting (set to 0 to disable). This job covers Jellyfin links only, filtered by connection type and by whether the connection is enabled -- not by confirmed/unconfirmed status; Emby artists are name-keyed rather than folder-backed, so a directory rename does not disturb their links in the first place, and Lidarr's links never enter the unconfirmed state this job repairs.
