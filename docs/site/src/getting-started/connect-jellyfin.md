...
In Stillwater, open **Settings** > **Connections** (or, during first-time setup, the Server Connections wizard step). Click **Add connection** and pick **Jellyfin**.
...
Click **Test connection**. Stillwater verifies the URL is reachable and the API key is valid. On success, it discovers the first administrator user on the server (used to scope subsequent metadata edits) and saves the connection.
...
- **Library import.** Stillwater pulls the list of album artists Jellyfin already knows about. Used to align Stillwater's library with Jellyfin's view; matters most when you want Stillwater to operate on the same artists Jellyfin has already organized.
- **Metadata push.** Stillwater pushes metadata edits to Jellyfin via its API after you make changes in Stillwater. Without this, edits stay local to Stillwater (and the NFO file, if NFO writeback is enabled).
...
To clear the gate when it does close, flip **Let Stillwater manage artwork and NFO files on this server** ON for the connection cited in the banner. You do not need to log into Jellyfin and change settings yourself. Stillwater handles the change for you:

1. Snapshots Jellyfin's current library options into the connection record so they can be restored later.
2. PATCHes Jellyfin's library options to disable its NFO saver and image saver (`SaveLocalMetadata=false`).
