...
- **Automatic link repair after a directory rename.** When you rename an artist's directory, Stillwater asks Jellyfin to rescan and then re-reads the artist back at its new path so its stored link keeps pointing at the right item. On a large library that rescan can take minutes, which is longer than the rename itself waits, so the link is left as-is and the rename is reported as needing attention. A scheduled background job (default cadence: every 15 minutes) picks those up afterward. It re-checks only the Jellyfin links still waiting to be confirmed, not every linked artist, and repairs one once Jellyfin's rescan has caught up. It only ever moves a link to an item Jellyfin itself reports at the artist's current path, and it never removes a link, so an artist that still cannot be matched is left alone and retried on the next pass. Configure the cadence via the `relink_reconcile.interval_minutes` setting (set to 0 to disable). This job covers Jellyfin links only; Emby artists are name-keyed rather than folder-backed, so a directory rename does not disturb their links in the first place.
