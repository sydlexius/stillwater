...
- __Connection shows "Connected" but `Last checked` is stale.__ Stillwater periodically re-checks each connection. If the timestamp is more than an hour old, restart Stillwater or trigger a manual check from the connections page.
