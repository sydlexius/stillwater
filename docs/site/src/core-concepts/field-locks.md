...
**All six provider ID fields are covered:** MusicBrainz, TheAudioDB, Discogs, Wikidata, Deezer, and Spotify. Pinning one stops the rules that adopt a MusicBrainz ID and backfill missing Discogs, Deezer, and Spotify IDs. The backfill rule now skips a pinned ID and says so, instead of reporting a fix it did not make: if that pinned field was the only thing it had to write, the rule's entry is closed out rather than left open with a Fix button that can only be refused again. You can still change a pinned ID yourself by editing the field directly; the link and identify flows, which choose a whole new identity for the artist, tell you the field is locked instead of overwriting it.
