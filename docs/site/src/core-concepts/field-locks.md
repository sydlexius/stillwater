...
**Provider ID fields are covered too.** Pinning a MusicBrainz, Discogs, Deezer, or Spotify ID stops the rules that adopt a MusicBrainz ID and backfill missing Discogs, Deezer, and Spotify IDs. You can still change a pinned ID yourself by editing the field directly; the link and identify flows, which choose a whole new identity for the artist, tell you the field is locked instead of overwriting it.
