...
<!-- ACCURACY NOTE (#3037, updated #3064/#3066): the persist chokepoint (internal/artist/lockguard.go) enforces per-field locks on the whole-row write every rule fixer ends in, for the artists-row fields AND the six provider IDs (the latter hydrated first, or the compare would restore an empty ID over a real one). Service.UpdateProviderField/ClearProviderField additionally REFUSE an ungranted write to a pinned provider ID with a typed *artist.FieldLockedError (internal/artist/field_lock_error.go) rather than letting the chokepoint revert it behind a nil return; the provider_id_missing fixer (internal/rule/fixers_provider_id.go) treats that refusal as a skip and leaves its row OPEN -- as of #3066, ALWAYS, even when a lock refused every still-missing field the violation covers. It no longer reports Dismissed for that case: a lock is operator-revocable state, not terminal, and a dismissed row has no un-dismiss route (UpsertViolation preserves 'dismissed' per #1107; ReopenViolation only accepts 'resolved'), so dismissing would mean the finding never returns once the operator unlocks. For every OTHER fixer -- which writes through the whole-row Service.Update/UpdateReportingLocks, where restore-and-continue means err==nil does NOT mean the change landed -- Service.UpdateReportingLocks / UpdateAfterRuleEvaluationReportingLocks return the restored field names, and internal/rule/lock_reverted_fix.go declines to report Fixed or resolve the row when EVERY guarded field that fixer changed was restored, leaving it OPEN too -- the two paths now agree. An open row keeps its paired rule_results FAIL row (UpsertViolation writes one only for open/pending per #1107), which is what keeps offlineHealthScore able to score the artist. The bulk fetch-images job's own MusicBrainz-ID self-heal (internal/rule/bulk_executor.go selfHealMBID, #3064) is guarded the same way: it calls UpdateReportingLocks rather than a bare Update, and skips both the in-memory adoption and the #2845 history row when musicbrainz_id comes back in the restored-field report. An operator editing a pinned provider ID is not blocked: the field-edit API carries a scoped grant, and every link/identify flow -- including handlers_discogs.go / handlers_audiodb.go, converged in #3037 -- refuses with 423. Still NOT guarded: the single-field verbs UpdateField/ClearField, deliberately, so history revert and blast-radius restore keep working; and "members", which lives in its own table. Do not claim those two are enforced. -->
...
**All six provider ID fields are covered:** MusicBrainz, TheAudioDB, Discogs, Wikidata, Deezer, and Spotify. Pinning one stops the rules that adopt a MusicBrainz ID and backfill missing Discogs, Deezer, and Spotify IDs. The backfill rule skips a pinned ID and says so, instead of reporting a fix it did not make. The entry stays open the same way any other lock-reverted rule's does -- even when every ID still missing on that artist was pinned -- because unlocking a field is all it takes to make the rule work again, and a closed-out entry never comes back. The Fix button does nothing until you unlock the field, whether you clicked it yourself or the rule ran on its own; the message names which fields are locked so it is clear why. You can still change a pinned ID yourself by editing the field directly; the link and identify flows, which choose a whole new identity for the artist, tell you the field is locked instead of overwriting it.
...
Each restored value appears in the artist's Recent Activity as an ordinary revert, and the pair drops out of the blast radius report because the field now holds your value again. The startup log summarizes the run: how many pairs were restored, how many were unrecoverable, and how many failed. The repair runs once per database. A run that hit a passing problem on some rows -- the database briefly unreadable, a write that could not land -- retries those rows at the next start, and rows repaired earlier are not touched again. A restore that is refused for a reason that would repeat identically every time (for example, putting the old value back would collide with another artist's name) is different: retrying cannot change that answer, so such rows are counted and reported in the log's own category and the repair still finishes rather than re-running forever.
