...
- **It only appears when a fanart push actually runs.** An artist Stillwater finds no readable top-level fanart for has nothing pushed, so nothing there can be cleared and rewritten, and no warning is raised.
