...
- **Jellyfin's upload endpoint still doesn't honor the slot it's given** -- that's a Jellyfin server limitation, not something Stillwater's client code controls, and it hasn't changed. For this one kind of replace, though, Stillwater now works around it: it clears every backdrop the connection has and re-uploads your full local set in order, so the count comes out the same and the primary slot's content is the one that changed. There's a brief moment mid-sync where Jellyfin has no backdrops for that connection at all, while the old set is cleared and the new one goes back up.
