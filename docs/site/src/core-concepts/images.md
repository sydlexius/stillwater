...
When you replace the primary fanart -- crop, re-fetch, or upload over the existing backdrop rather than adding a new one -- Stillwater pushes that change to Emby and Jellyfin differently, because the two platforms don't behave the same way once the update reaches them. This push only happens for a connection that's enabled, healthy, and has its **Image download/write** toggle on; a disabled, unhealthy, or image-write-off connection sees no push at all.
