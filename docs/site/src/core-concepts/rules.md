...
- **Enabled** -- whether the rule runs at all. Disabled rules don't evaluate and normally contribute nothing to violation counts. They stay listed in **Settings > Rules** so you can turn them back on. There is an exception for rules whose findings are recorded as they happen -- at the moment a matching event occurs -- rather than by a rule run: for these, disabling never stops recording, because nothing can recreate a missed finding afterward. Findings stay recorded, visible, counted, and fixable regardless of the toggle. What the toggle controls varies by rule: for Cross-artist backdrop collision it gates the pop-up notification shown at the moment a collision is detected -- disable it and you stop seeing the pop-up, but the finding is still recorded. See [enable and configure rules](../how-to/enable-and-configure-rules.md) for the current per-rule detail.
