...
- **Enabled** -- whether the rule runs at all. Disabled rules don't evaluate and normally contribute nothing to violation counts. They stay listed in **Settings > Rules** so you can turn them back on. There is one exception: a rule whose findings are recorded as they happen rather than by a rule run. Currently that is only Cross-artist backdrop collision. For this rule, disabling stops only the pop-up notification at the moment a collision is detected. Findings are still recorded, still visible, still counted, and still fixable exactly as before -- there is no way to recreate a missed one later, so recording never stops. Anything detected while the notification was off is still there once you re-enable it; you just did not see a pop-up for it in the moment. See [enable and configure rules](../how-to/enable-and-configure-rules.md).
