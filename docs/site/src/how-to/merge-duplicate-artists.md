...
- Stillwater asks every connected server to re-check the survivor and each merged-away (loser) artist, so a server that supports it notices the loser's directory is gone and drops the stale entry on its own. Lidarr and Emby/Jellyfin differ here: Jellyfin in particular can retain a leftover metadata-only artist even after this refresh, because of a Jellyfin-side limitation Stillwater can't work around -- see [Jellyfin keeps a leftover artist after a merge or rename](../troubleshooting/platform-auth.md#jellyfin-keeps-a-leftover-artist-after-a-merge-or-rename) for what to expect and how to clean it up.
