...
The container's healthcheck targets `localhost` over the configured protocol. When `SW_TLS_CERT_FILE` is set, the healthcheck derives its own URL and probes HTTPS on the right port. The probe runs with `wget --no-check-certificate` to skip cert verification -- this is intentional, because a localhost healthcheck against a self-signed cert would otherwise fail for the wrong reason. You do not need to set `SW_HEALTH_URL` yourself; setting `SW_TLS_CERT_FILE` is enough.
