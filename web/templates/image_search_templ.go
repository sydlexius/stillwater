...
func WebImageSearchResults(artistID string, images []provider.ImageResult, activeSort string, fanartExists bool) templ.Component {
	return templruntime.GeneratedTemplate(func(templ_7745c5c3_Input templruntime.GeneratedComponentInput) (templ_7745c5c3_Err error) {
		...
		if len(images) == 0 {
			...
			if templ_7745c5c3_Err != nil {
				return templ.Error{Err: templ_7745c5c3_Err, FileName: `web/templates/image_search.templ`, Line: 1578, Col: 102}
			}
			...
			if templ_7745c5c3_Err != nil {
				return templ.Error{Err: templ_7745c5c3_Err, FileName: `web/templates/image_search.templ`, Line: 1581, Col: 39}
			}
...
func openCropForType(imgSrc string, imageType string) templ.ComponentScript {
	return templ.ComponentScript{
		Name: `__templ_openCropForType_afa5`,
		Function: `function __templ_openCropForType_afa5(imgSrc, imageType){// Close the context menu before opening the crop modal so the dropdown
	// does not stay visible behind the modal overlay.
	//
	// #3223 review round 4, H3: same pre-existing bug as
	// swCloseAnyOpenContextMenu above -- this selector matched a live mobile
	// bottom sheet belonging to an UNRELATED ContextMenu elsewhere on the
	// page (it shares role="menu" and is never .hidden) and added .hidden to
	// it, which its own open/close logic never checks or clears, leaving it
	// permanently invisible until a full page reload. This editor's own
	// Actions menu never renders as a bottom sheet at any viewport width (it
	// is a hand-rolled dropdown, not components.ContextMenu), so the fix is
	// simply to exclude .ctx-bottom-sheet from this selector -- there is no
	// sheet of THIS menu's own that needs a separate close path, and closing
	// an unrelated one is exactly the bug being fixed, not a gap to fill in.
	var menus = document.querySelectorAll('[data-context-menu] [role=menu]:not(.hidden):not(.ctx-bottom-sheet)');
	for (var i = 0; i < menus.length; i++) {
		menus[i].classList.add('hidden');
		var trigger = menus[i].closest('[data-context-menu]').querySelector('[aria-haspopup]');
	...
	openCropModal(imgSrc, imageType, undefined, false);
		...
		Call:       templ.SafeScript(`__templ_openCropForType_afa5`, imgSrc, imageType),
		CallInline: templ.SafeScriptInline(`__templ_openCropForType_afa5`, imgSrc, imageType),
	}
}
