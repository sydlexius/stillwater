...
// TestArtistFindingsList_DoubleSubmitGuard pins the per-row double-submit
// guard. The artist-detail findings list uses raw fetch() rather than
...
// and re-enable them in finally(). We assert the rendered list contains
...
func TestArtistFindingsList_DoubleSubmitGuard(t *testing.T) {
	...
	// HTML for the list, so substring assertions are stable across both
	// emission modes.
	...
	if !strings.Contains(body, "actionBtns[i].disabled = true") {
		t.Errorf("findings-list fix/dismiss script missing the in-flight disable assignment; got:\n%s", body)
	}
	if !strings.Contains(body, "actionBtns[j].disabled = false") {
		t.Errorf("findings-list fix/dismiss script missing the finally() re-enable assignment; got:\n%s", body)
	}
	if !strings.Contains(body, ".finally(function()") {
		t.Errorf("findings-list fix/dismiss script missing finally() block; got:\n%s", body)
	}

	// User-facing failure surface: the script must route failures through
	// window.showToast so the findings list matches the dashboard toast
	// styling instead of using a native browser alert dialog.
	if !strings.Contains(body, "window.showToast") {
		t.Errorf("findings-list fix/dismiss script missing window.showToast routing; got:\n%s", body)
	}
}
