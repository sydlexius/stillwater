...
func (c *Client) GetMusicLibraries(ctx context.Context) ([]VirtualFolder, error) {
	...
	return mediabrowser.FilterMusicLibraries(folders, c.Logger, mediabrowser.PlatformEmby,
...
// LibraryScanIdle reports whether Emby currently has no library scan running.
// The companion to TriggerLibraryScan, which is fire-and-forget and so cannot
// tell a caller when its scan finished.
//
