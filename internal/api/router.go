...
func (r *Router) Handler(ctx context.Context) http.Handler {
	...
	mux.HandleFunc("GET "+bp+"/api/v1/providers/fields/{name}", wrapAuth(middleware.RequireAdmin(r.handleGetProviderFields), authMw))
	mux.HandleFunc("PUT "+bp+"/api/v1/providers/fields/{name}/{field}/toggle", wrapAuth(middleware.RequireAdmin(r.handleToggleProviderField), authMw))
